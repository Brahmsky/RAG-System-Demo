...
def multi_query_test(config: RAGConfig):
    ...
    print(rag.query("图灵有什么研究领域？文本和图谱都结合说说", mode="graph"))

def test_graph(config: RAGConfig):
    rag = RAGSystem(config)
    rag.add_corpus('test_build_graph.md', language="Chinese")
    print(rag.query("图灵被定成什么罪了？", mode="graph"))
    # rag.remove_corpus('test_build_graph.md')
